...
function LastSeen() {
    ...
    const deleteProject = (pId) =>  {
        axios.delete(`${baseUrl}/projects/${pId}`, {headers : {"Authorization" : "Bearer "+localStorage.getItem('accessToken')}})
            ...
            <div style={{ "height": "450px", "overflow": "auto" }} className="m-2">
                ...
                {Object.values(projects).map(key => (
                    <div className="card row m-2">
                        ...
                        <center><big key={key.id}><FaTrash onClick={() => deleteProject(key.id)} className="text-danger mr-2"/></big></center>
                    </div>
                ))}
            </div>
            }
